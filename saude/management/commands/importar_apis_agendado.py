...
class Command(BaseCommand):
    help = 'Executa importações de APIs configuradas (para agendamento)'

    def handle(self, *args, **options):
        self.stdout.write('Iniciando importações agendadas...')
        ...
        for config in configuracoes:
            if config.deve_executar():
                ...
                try:
                    ...
                    config.save()
                    executadas += 1
                    self.stdout.write(self.style.SUCCESS(f'  ✅ {config.nome} executado: {resultado["mensagem"]}'))
                except Exception as e:
                    config.ultimo_status = f"Erro: {str(e)}"
                    config.save()
                    self.stdout.write(self.style.ERROR(f'  ❌ Erro em {config.nome}: {str(e)}'))

        self.stdout.write(self.style.SUCCESS(f'✅ {executadas} APIs executadas.'))